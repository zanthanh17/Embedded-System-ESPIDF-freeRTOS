...
esp_rmaker_device_t *esp_rmaker_ph_sensor_device_create(const char *dev_name,
                                                        void *priv_data, float ph)
{
    esp_rmaker_device_t *device = esp_rmaker_device_create(dev_name, ESP_RMAKER_DEVICE_PH_SENSOR, priv_data);
    if (device)
    {
        esp_rmaker_device_add_param(device, esp_rmaker_name_param_create(ESP_RMAKER_DEF_NAME_PARAM, dev_name));
        esp_rmaker_param_t *primary = esp_rmaker_ph_param_create(ESP_RMAKER_DEF_PH_NAME, ph);
        esp_rmaker_device_add_param(device, primary);
        esp_rmaker_device_assign_primary_param(device, primary);
    }
    return device;
...
esp_rmaker_device_t *esp_rmaker_ntu_sensor_device_create(const char *dev_name,
                                                         void *priv_data, float ntu)
{
    esp_rmaker_device_t *device = esp_rmaker_device_create(dev_name, ESP_RMAKER_DEVICE_NTU_SENSOR, priv_data);
    if (device)
    {
        esp_rmaker_device_add_param(device, esp_rmaker_name_param_create(ESP_RMAKER_DEF_NAME_PARAM, dev_name));
        esp_rmaker_param_t *primary = esp_rmaker_ntu_param_create(ESP_RMAKER_DEF_NTU_NAME, ntu);
        esp_rmaker_device_add_param(device, primary);
        esp_rmaker_device_assign_primary_param(device, primary);
    }
    return device;
